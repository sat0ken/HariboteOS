...
; AT互換機の仕様ではPICの初期化をするなら、こいつをCLI前にやっておかないと、たまにハングアップする
; PICの初期化はあとでやる
...
; 残り全部
        ...
        SUB     ECX,512/4       ; IPLの文だけ差し引く
        CALL    memcpy

; asmheadでしなければならないことは全部終わったのであとはbootpackに任せる
; bootpackの起動
        MOV     EBX,BOTPAK
        MOV     ECX,[EBX+16]
        ADD     ECX,3           ; ECX += 3
...
waitkbdout:
        IN      AL,0x64
        AND     AL,0x02
        JNZ     waitkbdout      ; ANDの結果がOでなければwaitkbdoutへ
        RET
